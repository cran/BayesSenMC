...
The fit has an Akaike Information Criterion (AIC), which can be used to compare across models. The logit means of Se and Sp are given by the fixed effects, $\Sexpr{round(my.mod@beta[1] + my.mod@beta[2], 3)}$ and $\Sexpr{round(my.mod@beta[1], 3)}$, which translates to a sensitivity of $\Sexpr{round(exp(my.mod@beta[1] + my.mod@beta[2]) / (exp(my.mod@beta[1] + my.mod@beta[2]) + 1), 3)}$ and a specificity of $\Sexpr{round(exp(my.mod@beta[1]) / (exp(my.mod@beta[1]) + 1), 3)}$. The values, larger than 0.5, suggest that overall, the diagnostic accuracy for bipolar disorder given our meta-data is better than random, albeit nowhere near perfect. The standard deviations of logit Se and Sp are given by the random effects, as well as the correlation. All of the mentioned parameter estimates can be returned in a list by calling \code{paramEst}. 
...
\label{fig:trace}
\centering
<<echo = TRUE, results = hide, eval = FALSE, fig = TRUE>>=
